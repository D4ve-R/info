...
```js
IfcAPI.GetAllLines(modelID: number): Vector<number>
```
...
- `Vector<number>`: Vector of expressIDs
...
```js
IfcAPI.GetLine(modelID: number, expressID: number, flatten?: boolean): any
```

#### Arguments:

- `modelID`: Model handle retrieved by `OpenModel`.
- `expressID`: expressID of IFC entity.
- `flatten`: defaults to `false`, recursively resolve attributes that are nested IFC entities 

#### Example:

```js
const id = 123;
const entity = IfcAPI.GetLine(modelID, id);
// {expressID: 123, ..., ExampleAttr: {type: 5, value: 456}}

const entityFlat = IfcAPI.GetLine(modelID, id, true);
// {expressID: 123, ..., ExampleAttr: {expressID: 456, ...}}
```
...
```js
IfcAPI.GetLineIdsWithType(modelId: number, type: number, includeInherited?: boolean): Vector<number>
```

#### Arguments:

- `modelID`: Model handle retrieved by `OpenModel`
- `type`: type of IFC entity, see [here](https://github.com/IFCjs/web-ifc/blob/main/src/ifc-schema.ts)
- `includeInherited`: defaults to `false`

#### Returns: 

- `Vector<number>`: Vector of expressIDs with `type`
...
const lines = IfcAPI.GetLineIDsWithType(modelID, IFCWALL);

for(let i = 0; i < lines.size(); ++i){
  // expressID of IFC entity with type IFCWALL
